    ...
    if not data or not regions:
        ...
        if st.sidebar.button("Run Analysis"):
            with st.spinner("Calculating..."):
                res = httpx.post(f"{API_URL}/calculate_roi", json={"ecosystem_type": selected_key, "area_ha": area, "region": selected_region}).json()
                
                st.subheader(f"📊 Results: {res['ecosystem']}")
                c1, c2 = st.columns(2)
                c1.metric("Annual Value", f"₹{res['total_annual_value']:,.0f}")
                c2.metric("10-Year NPV", f"₹{res['npv_10yr']:,.0f}")
                
                # AI Insight Section
                ai_res = httpx.post(f"{API_URL}/ai_summary", json=res).json()
                st.info(f"🤖 **AI Insight:** {ai_res['summary']}")
                
                st.bar_chart(pd.DataFrame(res['breakdown']).set_index("service")["total_value"])
